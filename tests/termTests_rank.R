...
nws <- simulate(nw0~nonconformity("all")+nonconformity("local1")+nonconformity("local2")+deference, coef=rep(0,4),response="r", reference=~DiscUnif(1, n-1), nsim=S, statsonly=FALSE)

d.stats <- attr(nws,"stats")

c.stats <- t(sapply(lapply(nws,as.matrix,attrname="r"),function(m) c(global.nonconformity(m), local1.nonconformity(m), local2.nonconformity(m), deference(m))))

s.stats <- summary(nws~nonconformity("all")+nonconformity("local1")+nonconformity("local2")+deference, response="r")
...
nws <- simulate(nw1~nonconformity("all")+nonconformity("local1")+nonconformity("local2")+deference, coef=rep(0,4),response="r", reference=~DescRank, nsim=S, statsonly=FALSE)
